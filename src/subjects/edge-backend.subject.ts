...
export class EdgeBackendSubjectModule extends BaseSubjectModule {
  readonly subjectId = 'edge_backend'
  readonly displayName = 'Edge Backend'
  ...
  override getActiveCompatibility() {
    return this.releasesService.getActiveCompatibility()
  }

  override getCompatibility(version: string) {
    return this.releasesService.getCompatibility(version)
  }

  override upsertCompatibility(version: string, body: Record<string, unknown>) {
    return this.releasesService.upsertCompatibility(version, body)
  }
